...
export default function WhyWorkWithUs() {
  ...
  return (
    <section
      className=" relative w-full overflow-hidden bg-[#14052A] /* ✅ Hero background */ py-16 sm:py-20 md:py-24 "
    >
      {/* dotted background */}
      <div
        aria-hidden="true"
        className=" pointer-events-none absolute inset-0 opacity-[0.15] [background-image:radial-gradient(rgba(0,200,255,0.18)_1px,transparent_1px)] [background-size:40px_40px] [background-position:0_0,20px_20px] "
      />

      {/* soft cyan glow */}
      <div
        aria-hidden="true"
        className=" pointer-events-none absolute -bottom-24 right-0 w-[70vw] h-[70vw] md:w-[45vw] md:h-[45vw] bg-[radial-gradient(ellipse_at_center,rgba(0,200,255,0.20),transparent_60%)] blur-3xl "
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-10 md:gap-14 ">
          {features.map(({ Icon, title, text }, i) => (
            <div
              key={i}
              className=" flex flex-col items-center text-center bg-[#0b0b2e]/60 rounded-2xl p-6 shadow-md /* ✅ Restored inner card color */ transition-transform duration-300 hover:scale-[1.04] sm:bg-transparent sm:shadow-none sm:p-0 sm:rounded-none "
            >
              <Icon
                className={`${iconSize} text-cyan-400 drop-shadow-[0_0_12px_rgba(0,200,255,0.25)] transition-transform duration-300 hover:scale-110`}
                strokeWidth={2.2}
              />
